...
{
    private static readonly string[] pageTexts = new[]
    {
    ...
    private readonly ILogger<StoryPageController> _logger;
    
    public StoryPageController(ILogger<StoryPageController> logger)
    {
        _logger = logger;
    }

    [HttpGet]
    public IEnumerable<StoryPage> Get()
    {
        return Enumerable.Range(1, 5).Select(index => new StoryPage
        {
            pageNum = index,
            pageText = pageTexts[Random.Shared.Next(pageTexts.Length)]
        })
        .ToArray();
    }
}
